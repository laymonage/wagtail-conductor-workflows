...
1. **Reproduce.** Follow the reporter's "Steps to reproduce" literally.
   ...
   - If you need to write or modify files in the Wagtail source (for example, to run a candidate unit test from item 4), create a git worktree of the checkout inside the scratch dir and work there — the user's checkout at `{{ workflow.input.wagtail_dir }}` must stay pristine. Run any tests in the worktree using the `run-tests` skill.
